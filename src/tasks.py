...
from pdf import get_matching_page, write_page, parse_dates_from_delayed_salary, is_date_present_in_rlc_delay, \
    merge_pdfs, parse_regular_salary_type, get_matching_pages
...
def process_generic_rlc(rlc_type, salary_date, salary_file_path, rlc_folder_path, naf_dir, salary_output_path,
    ...
    try:
        ...
        log.error("Some of the RLC documents (N or P) has not been found. The salary file " + salary_file_path
                          + " will be skipped.")
        return
...
def process_rlc_l03(salary_file_path, salary_page_number, salary_page, salary_date, naf_dir, rlc_folder_path,
    ...
    log.info("Salary file " + salary_file_path + " page " +
    ...
    try:
        ...
        log.error("The delay date could not be parsed from the delay salary page. This document will be "
                          "skipped from search. The internal error is " + exc.__str__())
        return
    log.debug("Initial date is " + unparse_date(delay_initial_date, "-") + " and end date is " +
    ...
    while suffix < 100:  # Accepting only suffix under 100
        ...
        if is_date_present_in_rlc_delay(delay_initial_date, delay_end_date, rlc_path_n):
            ...
            if not os.path.exists(rlc_path_p):
                log.debug(f"Breaking out of the bucle because {rlc_path_p} does not exist.")
...
def process_salaries_with_rlc(salaries_folder_path, rlc_folder_path, naf_dir, naf, begin, end):
    ...
    for salary_file in salary_files_selected:
        ...
        for salary_page, salary_page_number in salary_pages:
            ...
            log.info(f"Detected NAF {str(naf)} in PDF salary {str(salary_file_path)}, page "
            ...
            if salary_type == SalaryType.DELAY:  # process L03 RLCs
                ...
                log.info(f"Salary file {salary_file_path} page {str(salary_page_number + 1)} has been selected "
                ...
                try:
                    ...
                    log.error(f"Salary file {salary_file_path} page {str(salary_page_number + 1)} is a type "
                ...
                if regular_salary_type == RegularSalaryType.MONTHLY:
                    log.info(f"Salary file {salary_file_path} page {str(salary_page_number + 1)} has been "
                    ...
                    log.error("The regular salary type is not recognized. This RLC will be skipped.")
                    continue
                ...
                log.error(f"Detected type {str(salary_type)} that is not a recognized type. The current salary "
        ...
        if (bank.__eq__("BBVA") or bank.__eq__("BBVA_endarreriments") or bank.__eq__("BBVA_endarreriments") or
            ...
            for file_name in file_names:
                try:
                    ...
                    log.debug("DNI " + str(naf_to_dni[naf]) + " not detected in " +
...
def process_contracts(contracts_folder_path, naf_dir, naf, begin, end):
    ...
    for contracts_file in contracts_files:
        ...
        if len(dates) == 3:  # Contract is temporary; has end date
            ...
            log.error("expected 3 fields in the name of the file " + contracts_file + " but " +
                              str(len(
...
def process_RNTs(rnts_folder_path, naf_dir, naf, begin, end):
    ...
    for rnt_file in rnt_files:
        ...
        if begin <= file_date <= end:
            ...
            log.info("RNT file " + rnt_path.__str__() + " is selected, because its date is " +
                             unparse_date(file_date) + ".")
            try:
                pages = get_matching_pages(rnt_path, naf.__str__(), r"\d{12}")
            except ValueError as e:
                log.debug("NAF " + naf.__str__() + " not detected in " + rnt_path + ". Error: " + e.__str__())
                continue
            ...
            log.debug("RNT file " + rnt_file.__str__() + " is not selected, because its date is " +
                              unparse_date(file_date) + ".")
...
def merge_rnts_rlcs(rnts_folder_path, rlcs_folder_path, naf_dir, begin, end):
    ...
    log.info("Generated months list from " + str(begin) + " to " + str(end) + " is: " + str(months_list))
    ...
    for current_date in months_list:
        ...
        log.debug("Output path: " + str(os.path.join(naf_dir, RNTS_AND_RLCS_OUTPUT_NAME, unparse_year_month(current_date) + "_Merged.pdf")))
        ...
        if len(paths_to_merge) >= 2:
            ...
            log.warning("During date " + unparse_year_month(current_date) + " there were less than one RNTs "
                                                                                    "or RLCs to "
